...
(function(){
  ...
  const defaultProfile=()=>({ name:'Pilot', sl:1500, rp:0, level:1, xp:0, battles:0, wins:0, kills:0, owned:['p40','bf109','yak1','spit','a6m','d520'], researched:[], selected:'p40', nation:'usa', settings:{vol:0.8,music:true,control:'mouse',quality:2,assist:'easy',arrows:'climb',gEffects:'on'}, sandbox:true, lastMode:'air', lastMap:'random', lastSize:'8', lastDiff:'1' });
  ...
  function applySettings(){ const s=prof.settings; Game.settings.control=s.control; Game.settings.quality=+s.quality; Game.settings.assist=s.assist||'easy'; Game.settings.arrows=s.arrows||'climb'; Game.settings.gEffects=s.gEffects||'on'; Game.settings.name=prof.name; Audio2.setVolume(s.vol); Audio2.setMusic(s.music); }
  ...
  // settings
  $('btnSettings').onclick=()=>{ Audio2.click(); const s=prof.settings; $('setVol').value=s.vol; $('setMusic').checked=s.music; $('setControl').value=s.control; $('setAssist').value=s.assist||'easy'; $('setArrows').value=s.arrows||'climb'; $('setG').value=s.gEffects||'on'; $('setQuality').value=s.quality; $('setName').value=prof.name; $('settingsModal').classList.remove('hidden'); };
  $('btnSettingsClose').onclick=()=>{ const s=prof.settings; s.vol=+$('setVol').value; s.music=$('setMusic').checked; s.control=$('setControl').value; s.assist=$('setAssist').value; s.arrows=$('setArrows').value; s.gEffects=$('setG').value; s.quality=$('setQuality').value; prof.name=$('setName').value.trim()||'Pilot'; save(); applySettings(); $('settingsModal').classList.add('hidden'); };
