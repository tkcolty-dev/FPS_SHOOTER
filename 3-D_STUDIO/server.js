...
app.post('/api/export', async (req, res) => {
  try {
    ...
    for (const u of proj.ui || []){
      if (u.url && u.url.startsWith('/models/')){ const f = path.join(MODEL_CACHE, path.basename(u.url)); if (fs.existsSync(f)){ const ext = path.extname(f).slice(1); u.url = 'data:image/' + (ext === 'jpg' ? 'jpeg' : ext) + ';base64,' + fs.readFileSync(f).toString('base64'); } }
    }
